...
public class ManageUserPage extends javax.swing.JFrame {
    ...
    private void btnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEditActionPerformed
        try {
            // Validate that all fields are filled before editing a row
            if (validateFields()) {
                DefaultTableModel model = (DefaultTableModel) tbUsersTable.getModel();
                int selectedRowIndex = tbUsersTable.getSelectedRow();
                ...
                if (selectedRowIndex != -1) { // Check if any row is selected
                    ...
                    String existingUserID = model.getValueAt(selectedRowIndex, 0).toString();
                    // No need to generate new UserID as it's an edit operation
                    String existingUserPW = model.getValueAt(selectedRowIndex, 1).toString();

                    // Update the values in the selected row
                    model.setValueAt(existingUserID, selectedRowIndex, 0);
                    model.setValueAt(existingUserPW, selectedRowIndex, 1);
                    ...
                    // Clear the text fields and combo box after editing the row
                    clearTextFields();
                } else {
